  ...
  head: [
    {
      id: 1,
      name: "Wizard Hat",
      filepath: require("../assets/images/hat.png"),
    },
...
const CharacterImage = ({
  ...
  filepath: ImageSourcePropType | string;
  part: BodyPart;
}) => {
  if (!filepath) {
    return (
...
export default function Profile() {
  ...
  const cyclePart = (part: BodyPart, direction: "next" | "prev") => {
    ...
    setSelectedParts((prev) => ({
      ...prev,
      [part]:
        (prev[part] + (direction === "next" ? 1 : -1) + partLength) %
        partLength,
    }));
  };
      ...
      <View style={styles.header}>
        <View style={styles.usernameContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              Alert.prompt("Edit Username", "Enter New Username", (value) =>
                rpc.auth.me
                  .$patch({ json: { username: value } })
                  .then(async (d) => {
      ...
      <View style={styles.characterPreview}>
        {Object.keys(characterParts).map((part) => (
          <View key={part} style={styles.partPreview}>
            <CharacterImage
              filepath={
                characterParts[part as BodyPart][
                  selectedParts[part as BodyPart]
                ].filepath
              }
